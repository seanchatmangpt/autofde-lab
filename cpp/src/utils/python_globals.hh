...
// This class enforces that python global data are read just after importing the
// module using them (i.e. in PYBIND11_MODULE). This is to prevent reading them
// on the fly within a parallel thread, which is not supported by pybind11
// (acquiring the GIL before reading global python objects has no effect and
// results in crashing the interpreter).
class Globals {
public:
  static void init() {
    if (!_initialized) {
  ...
  static void check_initialized() {
    if (!_initialized) {
      throw std::runtime_error(
          "Python globals not properly initialized. Call "
          "skdecide::Globals::init() in the PYBIND11_MODULE() macro.");
    }
  }
};
