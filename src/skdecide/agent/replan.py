...
:func:`~skdecide.powl.executor.enabled` again"; it does not mean the world was
...
:func:`~skdecide.powl.identity.activity_sha256` covers ``{label, action
...
why :class:`~skdecide.powl.identity.OccurrenceKey` is not a bare content hash.
...
from skdecide.powl.identity import OccurrenceKey, activity_sha256, node_id
...
class ReplanRefusal(StrEnum):
    ...
    Distinct from :class:`~skdecide.powl.refusals.PowlRefusal`: those describe an
...
def seed_marking(model1: PowlNode, pm: PreserveMap) -> Marking:
    ...
    :func:`~skdecide.powl.executor.enabled` at all. There is no guard for a
    caller to skip.
    ...
    Prior :class:`~skdecide.powl.identity.OccurrenceKey`\\ s are seeded into
