...
nothing in the world: an :class:`~skdecide.powl.algebra.Atom`'s ``action``
...
Re-entering a nested :class:`~skdecide.powl.algebra.ChoiceGraph` does **not**
...
:class:`~skdecide.powl.identity.OccurrenceKey` per fire, recording which
...
class Marking:
    ...
    #: :class:`~skdecide.powl.frequency.Frequency` is complete only when this
...
class ChoiceRecord:
    ...
    #: ``fire`` folds it into every :class:`~skdecide.powl.identity.OccurrenceKey`,
