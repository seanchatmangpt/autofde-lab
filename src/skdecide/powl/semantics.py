...
- ``max_unrolls`` caps how many times a single choice-graph node may be
  ...
  :class:`~skdecide.powl.frequency.Frequency`).

When a bound is hit this module **raises**
:class:`~skdecide.powl.refusals.PowlError` with
:data:`~skdecide.powl.refusals.PowlRefusal.BOUND_EXHAUSTED`. It never returns
...
def language(node: PowlNode, *, max_traces: int, max_unrolls: int) -> Language:
    ...
    Raises :class:`~skdecide.powl.refusals.PowlError` with
