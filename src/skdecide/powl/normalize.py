...
This makes **our** model digest-stable. It does **not** establish
    ...
    ``model_digest(skdecide) == digest(mfw) == digest(bcinr)``
...
-------------------------------------
* Every :class:`~skdecide.powl.algebra.PartialOrder` stores the transitive
...
* A partial order's children are sorted by their own recursive
  :func:`~skdecide.powl.identity.node_id`, and the edge relation is remapped
...
* A :class:`~skdecide.powl.algebra.ChoiceGraph`'s children are **not**
...
from skdecide.powl.identity import node_id
