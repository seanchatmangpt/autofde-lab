...
**This module must never import** :mod:`skdecide.powl.executor`, and it does
...
:mod:`skdecide.powl.algebra`, :mod:`skdecide.powl.identity` and
:mod:`skdecide.powl.refusals`.

Algorithm — no enumeration
--------------------------
Membership is decided structurally, not by generating the language:

* :class:`~skdecide.powl.algebra.PartialOrder` — the trace's label multiset
...
* :class:`~skdecide.powl.algebra.ChoiceGraph` — every child consumes a fixed,
...
* :class:`~skdecide.powl.algebra.Atom` — the trace is exactly its label.
* :class:`~skdecide.powl.algebra.Start`, ``End``, ``Silent`` — the empty trace.

Composite children are recursed into on their projected subtrace.

Documented limitation
---------------------
A composite carrying a :class:`~skdecide.powl.frequency.Frequency` other than
``ONCE`` makes the label multiset of the model non-static, and this check is
static by construction. Such a model raises
:data:`~skdecide.powl.refusals.PowlRefusal.IRREDUCIBLE_PROJECTION` rather than
...
from skdecide.powl.frequency import ONCE
...
def label_options(node: PowlNode) -> tuple[tuple[str, ...], ...] | None:
    ...
    This is what lets a :class:`~skdecide.powl.algebra.ChoiceGraph` nested
    inside a :class:`~skdecide.powl.algebra.PartialOrder` be decided at all:
