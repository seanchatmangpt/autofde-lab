...
(:class:`~skdecide.ocel.model.OCEL`) via :meth:`OcelLog.to_ocel2_json` and
...
* **Object attributes are time-versioned in the schema model, static in the
  ...
  :class:`~skdecide.ocel.model.ObjectChange` is emitted at its own time. On
...
:func:`skdecide.fabric.canonical.canonical_json` of the interchange
...
def _declared_type(kind: OcelValueKind) -> str:
    ...
    :class:`~skdecide.ocel.model.OcelValueKind` is the union of the two source
...
class OcelLog:
    ...
    def validate(self, *, strict_qualifiers: bool = False) -> "OcelLog":
        ...
        4. ``TIME_STABLE_ATTRIBUTE_CHANGED`` -- Definition 2, p. 6: for
           ...
           :class:`~skdecide.ocel.model.ObjectChange` naming one of these is
