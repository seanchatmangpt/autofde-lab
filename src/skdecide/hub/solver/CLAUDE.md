...
- Declare applicability structurally via `T_domain` — `get_domain_requirements()`
  (`src/skdecide/solvers.py:85`) derives the required domain characteristics from that MRO, and
...
2. **`match_solvers(..., ranked=True)` accepts the flag and ignores it**
   (`src/skdecide/utils.py:126`, `# TODO: implement ranking heuristic`); it always returns a
...
`src/skdecide/builders/solver/` (mixins), `src/skdecide/solvers.py` (base + applicability),
`src/skdecide/utils.py` (registry + matching), `hub/domain/` (subjects), `cpp/` (compiled
...
uv run python -m skdecide.fabric match --domain <Domain>
```
...
- If ranking is implemented, retract invariant 2 here, in `src/skdecide/CLAUDE.md`,
