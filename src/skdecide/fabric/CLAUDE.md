...
3. **CLI contract**: `python -m skdecide.fabric.pddl_engine <domain> <problem> <plan>` —
...
`skdecide.utils` (registry loaders), `hub/domain/pddl` + `cpp/src/hub/domain/pddl/semantics/`
...
uv run python -m skdecide.fabric.pddl_engine --help                 # must print "usage: ..."
uv run python -m skdecide.fabric.ontology ontology/skdecide-capabilities.ttl
```
