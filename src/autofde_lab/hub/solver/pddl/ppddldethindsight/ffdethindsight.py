...
try:
    from autofde_lab.hub.__skdecide_hub_cpp import (
        _FFDetHindsightSolver_ as CppFFDetHindsightSolver,
    )
    ...
    class FFDetHindsight(Solver, DeterministicPolicies, Utilities, FromAnyState):
        """FF-DetHindsight: hindsight optimization for PPDDL domains using
        FF as the inner deterministic planner.

        Counterpart of FFReplan: same PDDL effect-tree determinization
        but uses hindsight optimization (scenario sampling) instead of
        replanning.
        ...
        def __init__(
            ...
            epsilon: float = 1e-3,
            parallel: bool = False,
            callback: Callable[["FFDetHindsight"], bool] = lambda slv: False,
            verbose: bool = False,
        ) -> None:
            """Construct an FFDetHindsight solver instance.
        ...
        def _solve_from(self, memory: D.T_memory[D.T_state]) -> None:
            """Run the FF-DetHindsight algorithm from a given PDDL state.
            ...
            if not hasattr(self, "_cpp_solver"):
                ...
                if not hasattr(domain, "_task"):
                    raise TypeError(
                        "FFDetHindsight solver requires a PPDDLDomain "
                        "(with _task attribute)"
                    )
                self._task = domain._task
                self._cpp_solver = CppFFDetHindsightSolver(
                    self,
                    self._task,
                    self._parallel,
                    self._sample_width,
