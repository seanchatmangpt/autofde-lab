...
from typing import Any, Dict, FrozenSet, Iterable, Iterator, List, Optional, Sequence, Set, Tuple, Union
import rdflib
from rdflib.term import Identifier, Node, URIRef, BNode, Literal


# Type aliases for Datalog terms and atoms
Term = Union[Node, str]  # Can be an rdflib Node (URIRef, BNode, Literal) or variable / constant string
...
class DatalogAtom:
    ...
    def apply_binding(self, binding: Dict[str, Node]) -> DatalogAtom:
        ...
        new_pred = binding.get(str(self.predicate), self.predicate) if is_variable(self.predicate) else self.predicate
        new_args = tuple(
            binding.get(str(arg), arg) if is_variable(arg) else arg
            for arg in self.args
        )
    ...
    def to_rdf_triple(self) -> Optional[Tuple[Node, Node, Node]]:
        ...
        if len(self.args) == 2:
            ...
            o_node = o if isinstance(o, Node) else (URIRef(o) if str(o).startswith("http") else Literal(o))
...
class DatalogEngine:
    ...
    def __init__(self, rules: Optional[Sequence[DatalogRule]] = None, max_iterations: int = 1000):
    ...
    def execute_graph_fixpoint(
        ...
        """
        initial_facts = [
            DatalogAtom.from_rdf_triple((s, p, o))
            for s, p, o in graph
        ]
    ...
    def _match_body(
        ...
        for cand in candidates:
            ...
            if compatible:
                sub_results = self._match_body(body, body_idx + 1, new_binding, pred_index)
