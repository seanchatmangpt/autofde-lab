...
class AuthorityGrant:
    ...
    constraints: Dict[str, Any] = field(default_factory=dict)
    duties: Sequence[str] = field(default_factory=tuple)  # Required duty IDs or action IRIs
...
class AuthorityBroker:
    ...
    def __init__(
        ...
        self._grants: Dict[str, AuthorityGrant] = {g.grant_id: g for g in (grants or [])}
    ...
    def evaluate(self, request: ConsequenceRequest) -> AuthorityDecision:
        ...
        for pol in self._policies:
            for prohib in pol.prohibitions:
                if self._matches_rule(prohib, request.actor_id, request.action_iri, request.target_resource):
        ...
        if grant is not None:
            ...
            if grant.action_iri != request.action_iri or grant.target_resource_iri != request.target_resource:
                return AuthorityDecision(
                    authorized=False,
                    grant_id=grant.grant_id,
                    refusal_code=REFUSED_NO_GRANT,
        ...
        for pol in self._policies:
            for perm in pol.permissions:
                if self._matches_rule(perm, request.actor_id, request.action_iri, request.target_resource):
                    ...
                    for duty in perm.duties:
                        duty_action = duty.action.value if isinstance(duty.action, OdrlAction) else str(duty.action)
                        if duty.uid not in fulfilled_duties and duty_action not in fulfilled_duties:
                    ...
                    return AuthorityDecision(
                        ...
                        refusal_code=None,
                        constraints={c.left_operand: c.right_operand for c in perm.constraints},
                    )
    ...
    def _matches_rule(self, rule: Permission | Prohibition, actor_id: str, action: str, target: str) -> bool:
        assignee_id = rule.assignee.uid if hasattr(rule.assignee, "uid") else str(rule.assignee)
        target_id = rule.target.uid if hasattr(rule.target, "uid") else str(rule.target)
        rule_action = rule.action.value if isinstance(rule.action, OdrlAction) else str(rule.action)
    ...
    def _check_constraints(self, constraints: Sequence[Constraint], context: Dict[str, Any]) -> bool:
