...
class CompositePlannerEngine:
    ...
    def run_diagnosis(
        ...
        if missing_objects:
            objs_str = ", ".join(
                f"{mo.kind}/{mo.object_name} ({mo.reason})" for mo in missing_objects
            )
            text_parts.append(
                f"Detected missing or corrupted Kubernetes objects in namespace {self.namespace}: {objs_str}."
            )

        if probe_faults:
            pf_str = ", ".join(
                f"{pf.deployment_name}:{pf.probe_type} ({pf.fault_kind})"
                for pf in probe_faults
            )
            text_parts.append(
                f"Detected probe misconfigurations in namespace {self.namespace}: {pf_str}."
            )

        if flagd_drift and flagd_drift.has_drift:
            flags_str = ", ".join(
                f"{df.flag_name}='{df.current_variant}'"
                for df in flagd_drift.drifted_flags
            )
            text_parts.append(
                f"Detected flagd feature flag config drift in namespace {self.namespace}: {flags_str}."
            )

        if trace_anomalies and trace_anomalies.has_anomaly:
            text_parts.append(
                f"Detected OTel trace RPC anomalies: {trace_anomalies.reasoning}"
            )

        if ingress_misroutes:
            ing_str = ", ".join(
                f"{ing.ingress_name} path {ing.path} -> {ing.observed_backend_service}"
                for ing in ingress_misroutes
            )
            text_parts.append(
                f"Detected Ingress backend misroutes in namespace {self.namespace}: {ing_str}."
            )

        if target_port_faults:
            tp_str = ", ".join(
                f"{tp.service_name} targetPort={tp.observed_target_port}"
                for tp in target_port_faults
            )
            text_parts.append(
                f"Detected Service targetPort mismatches in namespace {self.namespace}: {tp_str}."
            )

        if cronjob_mutations:
            cj_str = ", ".join(
                f"CronJob {cj.cronjob_namespace}/{cj.cronjob_name} -> victim {cj.victim_deployment}"
                for cj in cronjob_mutations
            )
            text_parts.append(
                f"Detected mutator CronJobs targeting deployments: {cj_str}."
            )

        if scheduling_deadlocks:
            sd_str = ", ".join(
                f"{sd.deployment_name} ({sd.constraint_type})"
                for sd in scheduling_deadlocks
            )
            text_parts.append(
                f"Detected scheduling deadlocks in namespace {self.namespace}: {sd_str}."
            )

        if coredns_faults:
            dns_str = ", ".join(
                f"{cd.configmap_name} ({cd.fault_kind})" for cd in coredns_faults
            )
            text_parts.append(
                f"Detected CoreDNS configuration faults in kube-system: {dns_str}."
            )

        if workload_misconfigs:
            wm_str = ", ".join(
                f"{wm.deployment_name} ({wm.fault_kind})" for wm in workload_misconfigs
            )
            text_parts.append(
                f"Detected workload/rolling update misconfigurations in namespace {self.namespace}: {wm_str}."
            )
        ...
        if dns_policy_overrides:
            ...
            text_parts.append(
                f"Detected Deployment DNS policy overrides in namespace {self.namespace}: {dp_str}."
            )
        ...
        if host_port_conflicts:
            ...
            text_parts.append(
                f"Detected container hostPort binding conflicts in namespace {self.namespace}: {hp_str}."
            )
        ...
        if pvc_claim_mismatches:
            ...
            text_parts.append(
                f"Detected dangling PVC claim references in namespace {self.namespace}: {pcm_str}."
            )
        ...
        if pvc_multi_attach_faults:
            ...
            text_parts.append(
                f"Detected PVC multi-attach conflicts in namespace {self.namespace}: {pma_str}."
            )

        if rbac_misconfigs:
            rbac_str = ", ".join(
                f"{rf.deployment_name}/{rf.service_account_name} ({rf.fault_kind})"
                for rf in rbac_misconfigs
            )
            text_parts.append(
                f"Detected RBAC misconfigurations in namespace {self.namespace}: {rbac_str}."
            )
        ...
        if resourcequota_exhaustions:
            ...
            text_parts.append(
                f"Detected ResourceQuota exhaustion in namespace {self.namespace}: {rq_str}."
            )
        ...
        if limitrange_violations:
            ...
            text_parts.append(
                f"Detected LimitRange violations in namespace {self.namespace}: {lr_str}."
            )

        if not text_parts:
            diagnosis_text = (
                f"No fault mechanism anomalies detected in namespace {self.namespace}."
            )
        else:
