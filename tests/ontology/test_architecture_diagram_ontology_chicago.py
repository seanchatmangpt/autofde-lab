...
def test_every_catalog_entry_is_identity_bound_to_its_exact_mermaid_source() -> None:
    g = _merged_catalog()
    by_path = {
        str(g.value(d, ARCH.sourcePath)): d
        for d in set(g.subjects(RDF.type, ARCH.Diagram))
    }
    ...
    for src in _sources():
        ...
        diagram = by_path[rel]
        assert (
            str(g.value(diagram, ARCH.sourceDigest))
            == hashlib.sha256(src.read_bytes()).hexdigest()
        )
        assert str(g.value(diagram, ARCH.parseStanding)) == "PARSED"


def test_detailed_parser_materializes_real_elements_and_relationships_for_all_sources() -> (
    None
):
...
def test_core_vocabulary_and_shacl_shapes_are_parseable() -> None:
    ...
    Graph().parse(
        ROOT / "ontology" / "shapes" / "architecture.shacl.ttl", format="turtle"
    )
