...
---------------------------------
``skdecide/powl/__init__.py`` is a flat re-export: it eagerly imports every
submodule, semantics included. So a naive ``import skdecide.powl.validate``
always drags ``skdecide.powl.semantics`` in *through the package* regardless of
what ``validate.py`` itself imports, and the test would be unfalsifiable. The
probe therefore installs a stub ``skdecide.powl`` package module (correct
...
def test_validate_does_not_import_executor_or_semantics():
    _assert_separated("skdecide.powl.validate")


def test_membership_does_not_import_executor_or_semantics():
    if not _module_exists("skdecide.powl.membership"):
        pytest.xfail("skdecide.powl.membership does not exist yet")
    _assert_separated("skdecide.powl.membership")


def test_probe_would_catch_a_violation():
    """The probe is falsifiable: a module that *does* import semantics fails it."""
    if not _module_exists("skdecide.powl.semantics"):
        pytest.xfail("skdecide.powl.semantics does not exist yet")
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
    ...
    assert "FORBIDDEN_IMPORT:skdecide.powl.semantics" in proc.stderr
