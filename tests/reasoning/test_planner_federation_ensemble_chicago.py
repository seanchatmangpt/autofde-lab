...
def test_zero_solver_names_returns_empty_dict() -> None:
    assert (
        federate_concurrently(
            domain_path=BLOCKS3_DOMAIN, problem_path=BLOCKS3_PROBLEM, solver_names=()
        )
        == {}
    )
...
def test_concurrent_result_matches_sequential_federate_shape_and_content() -> None:
    ...
    sequential = federate(
        domain_path=BLOCKS3_DOMAIN, problem_path=BLOCKS3_PROBLEM, timeout_s=30.0
    )
    concurrent = federate_concurrently(
        domain_path=BLOCKS3_DOMAIN,
        problem_path=BLOCKS3_PROBLEM,
        timeout_s=30.0,
        max_workers=2,
    ...
    assert {k: v is not None for k, v in sequential.items()} == {
        k: v is not None for k, v in concurrent.items()
    }
    for name in SOLVER_NAMES:
        if sequential[name] is not None:
            ...
            assert [s.label for s in sequential[name].children] == [
                s.label for s in concurrent[name].children
            ]


def test_a_real_unregistered_solver_still_returns_none_without_breaking_others() -> (
    None
):
