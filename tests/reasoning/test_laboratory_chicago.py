...
def test_unsupported_process_science_provider_is_real_and_honest() -> None:
    ...
    obs = EnterpriseObservation(
        ontology_graph_ref="o", source_provenance_ref="s", enterprise_world_ref="w"
    )
...
def test_infer_desired_state_hypotheses_returns_one_real_rule_based_hypothesis_by_default() -> (
    None
):
...
def test_infer_desired_state_hypotheses_adds_a_second_hypothesis_only_when_process_observation_is_real() -> (
    None
):
    metadata = ScenarioMetadata_checkout_latency_scenario_v_1()
    ...
    real = ProcessObservation(
        evidence_standing="ALIVE", performance_metric_refs=("metric-1",)
    )
...
def test_classify_operator_applicability_admits_known_shapes_and_unknowns_unmapped_ones() -> (
    None
):
    results = classify_operator_applicability(
        ("hard_constraints", "some_novel_shape_no_one_mapped")
    )
    ...
    unknown = next(
        r for r in results if r.operator_class == "some_novel_shape_no_one_mapped"
    )
...
def test_falsify_candidate_is_unknown_with_zero_receipts_never_survives_by_default() -> (
    None
):
...
def test_falsify_candidate_is_falsified_when_a_real_receipt_reports_a_violation() -> (
    None
):
    candidate = ArchitectureCandidate(
        candidate_id="cand-1", target_state_assertions=("p95<250ms",)
...
def test_falsify_candidate_survives_only_when_real_receipts_confirm_postconditions_with_no_violations() -> (
    None
):
    candidate = ArchitectureCandidate(
        candidate_id="cand-1", target_state_assertions=("p95<250ms",)
...
def test_architecture_change_trigger_only_fires_above_its_real_confidence_threshold() -> (
    None
):
    low = ArchitectureChangeTrigger(
        evidence_refs=("ev-1",),
        detected_drift="latency regression",
        affected_requirement_refs=("req-1",),
        confidence=0.2,
        trigger_policy="threshold-0.5",
    )
