...
def test_stress_b13_multiple_configmaps_and_secrets_drift():
    ...
    configmaps = {
        "items": [
            ...
            {
                "metadata": {"name": "rate-config", "namespace": "hotel-reservation"},
                "data": {"config.json": "{}"},
            },
        ]
    }
...
def test_stress_multi_port_service_targetport_indexing():
    ...
    ing_faults, tp_faults = detect_ingress_and_targetport_faults(
        services_json=services, namespace="default"
    )
    ...
    cmds, deps = decide_ingress_targetport_remediation_commands(
        ing_faults, tp_faults, namespace="default"
    )
...
def test_stress_multi_container_pod_resource_request_removal():
    ...
    faults = detect_workload_and_rolling_update_misconfigs(
        deployments_json=deployments, namespace="default"
    )
    assert len(faults) == 1
    ...
    patch_cmds = [
        c for c in cmds if "kubectl patch deployment" in c and "resources/requests" in c
    ]
    assert len(patch_cmds) == 1
...
def test_stress_multi_container_cronjob_mutation_removal():
    ...
    faults = detect_cronjob_mutations(
        cronjobs_json=cronjobs, deployments_json=deployments, namespace="default"
    )
    assert len(faults) == 1
    ...
    patch_cmds = [
        c for c in cmds if "kubectl patch deployment" in c and "limits/memory" in c
    ]
    assert len(patch_cmds) == 1
    assert "/spec/template/spec/containers/1/resources/limits/memory" in patch_cmds[0]
    assert (
        "/spec/template/spec/containers/0/resources/limits/memory" not in patch_cmds[0]
    )
        ...
        "items": [
            {
                ...
                "spec": {
                    "template": {
                        "spec": {
                            "containers": [
                                {"name": "custom", "ports": [{"containerPort": 8080}]}
                            ]
                        }
                    }
                },
                    ...
                    "template": {
                        "spec": {
                            "containers": [
                                {
                                    "name": "recommendation",
                                    "resources": {"limits": {"memory": "8192Ki"}},
                                }
                            ]
                        }
                    }
...
def test_stress_rolling_update_without_init_container():
    ...
    print(
        "\n[STRESS TEST OBSERVED] Rolling Update Remediator commands without initContainers:",
        cmds,
    )
        ...
        "items": [
            {
                "metadata": {"name": "coredns", "namespace": "kube-system"},
                "data": {
                    "Corefile": "template ANY ANY svc.cluster.local { rcode NXDOMAIN }"
                },
            }
        ]
    }
