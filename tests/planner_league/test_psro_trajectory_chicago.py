...
def test_real_multi_round_trajectory_converges_toward_the_dominant_response() -> None:
    ...
    trajectory = run_psro_trajectory(
    ...
    assert all(
        step.receipt.selected_best_response == "Astar" for step in trajectory.steps
    )
    ...
    for bad_value in (0, -1):
        try:
            run_psro_trajectory(
                oracle,
                initial_state,
                candidates=("AOstar", "Astar"),
                max_rounds=bad_value,
            )
...
def test_single_round_trajectory_matches_a_bare_oracle_step_call() -> None:
    ...
    trajectory = run_psro_trajectory(
        oracle, initial_state, candidates=("AOstar", "Astar"), max_rounds=1
    )
    direct_step = oracle.step(initial_state, candidates=("AOstar", "Astar"))

    assert len(trajectory.steps) == 1
    assert (
        trajectory.steps[0].receipt.identity_sha256
        == direct_step.receipt.identity_sha256
    )
