...
def test_schedules_real_cross_play_for_the_real_identity_degradation_world_deterministically() -> (
    None
):
    league = PlannerLeague()
    ...
    assert [m.identity_sha256 for m in result_a.matches] == [
        m.identity_sha256 for m in result_b.matches
    ]


def test_non_default_worlds_produce_a_real_distinct_schedule_from_generic_enterprise() -> (
    None
):
