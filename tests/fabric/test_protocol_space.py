from autofde_lab.fabric.protocol_space import ProtocolCandidate,ProtocolConstraints,admit,maximal_admissible_space

def constraints():
    return ProtocolConstraints(allowed_languages=("rust","python"),allowed_transports=("stdio","http"),allowed_digests=("sha-256","blake3-256"),allowed_persistence=("sqlite","append-log"),authority_providers=("external-a","external-b"))
...
def test_planner_annotations_cannot_create_authority():
    bad=ProtocolCandidate("rust","stdio","sha-256","sqlite","planner",True,ambient_authority=True)
