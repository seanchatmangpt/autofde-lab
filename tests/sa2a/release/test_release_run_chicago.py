...
def test_full_crown_reaches_crowned_through_every_required_predecessor(
    tmp_path: Path,
) -> None:
    ...
    assert (
        result.fresh_consumer_standing["verdict"] == "CONFORMANT_EVIDENCE_RECONSTRUCTED"
    )


def test_episode_2_receives_the_real_composition_digest_not_an_empty_string(
    tmp_path: Path,
) -> None:
    ...
    assert result.exact_subject is not None
    assert (
        result.episode1.episode.exact_subject_digest
        == result.exact_subject.composition_digest
    )
    assert (
        result.episode2.episode.exact_subject_digest
        == result.exact_subject.composition_digest
    )
...
def test_both_episodes_carry_a_real_manufacture_digest(tmp_path: Path) -> None:
    ...
    assert (
        result.episode1.episode.manufacture_digest
        == result.episode2.episode.manufacture_digest
    )
...
def test_dirty_worktree_subject_is_blocked_before_episode1_ever_runs(
    tmp_path: Path,
) -> None:
    ...
    dirty_manifest = {
        **_MANIFEST,
        "repositories": [{"name": "autofde-lab", "exact_sha": f"dirty:{'a' * 40}"}],
    }
...
def test_state_machine_refuses_skipping_a_required_predecessor() -> None:
    ...
    assert not can_transition_release(
        ReleaseState.CREATED, ReleaseState.EPISODE_1_RUNNING
    )
...
def test_fresh_consumer_is_a_genuinely_separate_process_not_an_in_process_call(
    tmp_path: Path,
) -> None:
