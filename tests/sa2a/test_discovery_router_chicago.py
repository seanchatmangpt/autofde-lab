...
def _candidate(engine_id: str, query: UnknownQuery) -> CandidateResolution:
    return CandidateResolution(
        ...
        proposed_assertion=f"resolved-by:{engine_id}",
...
def test_falls_through_precedence_when_earlier_engines_cannot_handle_the_query() -> (
    None
):
    router = DiscoveryRouter()
    router.register(
        DiscoveryEngine(
            "exact-1", DiscoveryEngineKind.EXACT_REUSABLE_MACHINERY, lambda q: None
        )
    )
    router.register(
        DiscoveryEngine(
            "formal-1", DiscoveryEngineKind.FORMAL_PLANNER_OR_SOLVER, lambda q: None
        )
    )
    router.register(
        DiscoveryEngine(
            ...
            lambda q: _candidate("general-1", q),
        )
    )
    result = router.route(UnknownQuery(query_id="q-1", predicate_or_topic="anything"))
...
def test_duplicate_engine_id_is_refused() -> None:
    ...
    router.register(
        DiscoveryEngine(
            "dup", DiscoveryEngineKind.EXACT_REUSABLE_MACHINERY, lambda q: None
        )
    )
    try:
        router.register(
            DiscoveryEngine(
                "dup", DiscoveryEngineKind.FORMAL_PLANNER_OR_SOLVER, lambda q: None
            )
        )
        assert False, (
            "must refuse a second engine registered under an already-used engine_id"
        )
    except ValueError:
        pass
...
def test_wrong_type_return_falls_through_to_a_later_valid_engine() -> None:
    ...
    router.register(
        DiscoveryEngine(
            "int-1", DiscoveryEngineKind.EXACT_REUSABLE_MACHINERY, lambda q: 42
        )
    )
    router.register(
        DiscoveryEngine("dict-1", DiscoveryEngineKind.COMPOSITION, lambda q: {})
    )
    router.register(
        DiscoveryEngine(
            "formal-1",
            DiscoveryEngineKind.FORMAL_PLANNER_OR_SOLVER,
            lambda q: _candidate("formal-1", q),
        )
    )
    result = router.route(UnknownQuery(query_id="q-1", predicate_or_topic="anything"))
...
def test_wrong_type_return_never_reaches_episode1runner_as_a_fake_candidate(
    tmp_path,
) -> None:
...
def test_cli_requires_port_router_prefers_exact_machinery_for_the_demonstrated_query() -> (
    None
):
    ...
    assert result.selected_kind == DiscoveryEngineKind.EXACT_REUSABLE_MACHINERY
    assert not result.used_general_exploratory_intelligence
    assert invocation_log == ["exact-port-probe"], (
        "general fallback must not be invoked when exact machinery answers"
    )


def test_cli_requires_port_router_falls_back_to_general_for_an_unrecognized_topic() -> (
    None
):
    ...
    result = router.route(
        UnknownQuery(
            query_id="q-2", predicate_or_topic="service:x requires-replication-factor"
        )
    )
...
def test_crown_cli_command_actually_selects_exact_machinery_not_the_fallback(
    tmp_path,
) -> None:
    ...
    assert (
        payload["episode2_generated_candidate"]["generated_from_seed_candidate_id"]
        == "cand-ep1"
    )


def test_episode1_cli_command_actually_selects_exact_machinery_not_the_fallback(
    tmp_path,
) -> None:
