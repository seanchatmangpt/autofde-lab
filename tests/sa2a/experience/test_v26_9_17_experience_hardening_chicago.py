...
from autofde_lab.sa2a.experience.qualification import (
    REFUSED_ALREADY_QUALIFIED,
    ExperienceQualifier,
)
...
def _admitted_candidate(
    candidate_id: str = "cand-h1", query_id: str = "q-h1"
...
def test_compile_refuses_typed_valueerror_when_candidate_hash_cannot_be_computed() -> (
    None
):
    ...
    candidate = CandidateResolution(
        candidate_id="cand-nonserializable",
        query_id="q-1",
        proposed_assertion="x",
        evidence_payload={
            "weird": {1, 2, 3}
        },  # a set: json.dumps cannot serialize this
    ...
    receipt = AdmissionReceipt(
        receipt_id="rec-1",
        candidate_hash="precomputed-not-from-property",
        admitted=True,
        epistemic_standing=EpistemicState.KNOWN,
        reasons=("OK",),
        admitted_assertion="x",
    )
...
def test_qualify_called_twice_on_same_admitted_object_refuses_second_call_not_a_silent_double_registration() -> (
    None
):
    ...
    active_routes = [
        r for r in routes.routes_for_class("requires-port") if r.state == "ACTIVE"
    ]
    assert len(active_routes) == 1, (
        "exactly one ACTIVE route must survive a repeated qualify() call"
    )
...
def test_known_route_registry_allows_requalification_after_invalidation() -> None:
    ...
    experience = compiler.compile(
        ...
        equivalence_predicate_id="pred-1",
        invalidation_set={"ontology_digest": "v1"},
    )
    gate = ExperienceAdmissionGate(compiler.artifact_registry)
    admitted = gate.admit(experience)
    routes = KnownRouteRegistry()
    ...
    invalidation = check_and_invalidate(
        first.experience, {"ontology_digest": "v2"}, routes
    )
    ...
    second = qualifier.qualify(
        re_admitted, equivalence_predicate=lambda c: True, probe_input="requires-port"
    )
...
def test_lookup_timing_scales_with_routes_in_the_target_class_not_total_routes() -> (
    None
):
    ...
    def _accept_everything(_candidate: object) -> bool:
        return (
            False  # never matches -- forces lookup() to scan every route in the class
        )
    ...
    for class_idx in range(n_classes):
        ...
        for route_idx in range(routes_per_class):
            routes.register_route(
                KnownRoute(
                    route_id=f"route-{class_id}-{route_idx}",
                    semantic_class_id=class_id,
                    experience_id=f"exp-{class_id}-{route_idx}",
                    equivalence_predicate_id=predicate_id,
                    required_preconditions=(),
                    planner_or_policy_identity="p",
                    manufacturer_identity="m",
                    expected_capabilities=(),
                    resource_envelope={"max_probe_calls": 1},
                    qualification_receipt=f"rcpt-{class_id}-{route_idx}",
                    state="ACTIVE",
                )
            )
    ...
    assert (
        result is None
    )  # the always-False predicate means UNKNOWN is the correct outcome
    ...
    assert elapsed < 2.0, (
        f"lookup() took {elapsed:.3f}s for {n_lookups} calls -- looks O(all routes), not O(class)"
    )
...
def test_empty_current_digests_against_a_nonempty_invalidation_set_reports_invalidated() -> (
    None
):
    ...
    experience = compiler.compile(
        ...
        equivalence_predicate_id="pred-1",
        invalidation_set={"ontology_digest": "v1", "rule_digest": "v2"},
    )
    gate = ExperienceAdmissionGate(compiler.artifact_registry)
    admitted = gate.admit(experience)
    routes = KnownRouteRegistry()
    ...
    route = [
        r
        for r in routes.routes_for_class("requires-port")
        if r.route_id == qualified.known_route.route_id
    ][0]
    assert route.state == "INVALIDATED"


def test_empty_invalidation_set_is_never_invalidated_by_an_empty_current_digests() -> (
    None
):
    ...
    exp = MachineExperience(
        experience_id="e1",
        semantic_class_id="c1",
        source_episode_id="ep1",
        source_candidate_digest="d1",
        source_admission_receipt="r1",
        discovery_identity="di",
        discovery_resource_receipt="dr",
        solution_candidate_digest="d1",
        solution_admission_receipt="r1",
        compiled_artifact_ids=("a1",),
        equivalence_predicate_id="p1",
        invalidation_set={},
...
def test_artifact_registry_store_refuses_a_genuine_rule_id_collision() -> None:
    ...
    rule_a = CompiledDeterministicRule(
        rule_id="rule_collision01",
        pattern="pattern-A",
        deterministic_output="output-A",
        fingerprint="fpA",
    )
    rule_b = CompiledDeterministicRule(
        rule_id="rule_collision01",
        pattern="pattern-B",
        deterministic_output="output-B",
        fingerprint="fpB",
    )
    ...
    try:
        ...
        assert False, (
            "store() must refuse a genuine key collision with different content, never silently overwrite"
        )
    except ValueError as exc:
...
def test_artifact_registry_store_is_idempotent_for_byte_identical_content() -> None:
    ...
    rule_again = CompiledDeterministicRule(
        rule_id="rule_idempotent",
        pattern="pattern-A",
        deterministic_output="output-A",
        fingerprint="fpA",
    ...
    registry.store(
        rule_again
    )  # different object, byte-identical content -- must not raise
