...
def test_receipt_store_construction_stays_bounded_at_200_preexisting_files(
    tmp_path: Path,
) -> None:
    ...
    grant = AuthorityGrant(
        ...
        subject_id="smoke-actor",
        action_iri="urn:action:smoke",
        target_resource_iri="urn:cap:smoke-target",
    )
    ...
    admission = admit_target_binding(
        "urn:action:smoke",
        "urn:cap:smoke-target",
        issuer="smoke-actor",
        timestamp="2026-09-17T00:00:03Z",
    )
    for i in range(
        100
    ):  # 100 actuations -> 100 prep_*.json + 100 final_*.json = 200 files
        envelope = ExecutionEnvelope(
            idempotency_token=f"smoke-act-{uuid.uuid4().hex[:12]}-{i}",
            action_iri="urn:action:smoke",
            target_resource="urn:cap:smoke-target",
            actor_id="smoke-actor",
            grant_id=grant.grant_id,
            plan_digest=f"smoke-plan-{i}",
