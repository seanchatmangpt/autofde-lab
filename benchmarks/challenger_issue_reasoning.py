...
CASES: tuple[CaseStudy, ...] = (
    CaseStudy(
        "global_platform_reliability",
        "Why purchase fresh cognition for recurring platform invariants?",
        (
            ("workload_pending",),
            ("restarting",),
            ("no_endpoints",),
    ...
    CaseStudy(
        "zero_trust_access",
        "Why escalate bounded policy-graph failures to open-ended reasoning?",
        (
            ("authorization_denied",),
            ("policy_violation",),
            ("authorization_denied", "configuration_drift"),
            ("policy_violation", "authorization_denied"),
            ("authorization_denied",),
            ("policy_violation",),
            ("authorization_denied",),
            ("novel_or_metastable",),
    ...
    CaseStudy(
        "cloud_migration_factory",
        "Why reason manually about repeatable migration incompatibilities?",
        (
            ("configuration_drift",),
    ...
    CaseStudy(
        "data_platform",
        "Why send deterministic data-contract failures through expensive cognition?",
        (
            ("schema_validation_failure",),
            ("queue_lag",),
            ("dependency_unreachable",),
    ...
    CaseStudy(
        "software_delivery",
        "Why make engineers rediscover known delivery failure topology?",
        (
            ("build_failure",),
            ("version_incompatible",),
            ("configuration_drift",),
    ...
    CaseStudy(
        ...
        "Why treat bounded integration breakage as a language-generation problem?",
        (
            ("dependency_unreachable",),
            ("schema_validation_failure",),
            ("version_incompatible",),
    ...
    CaseStudy(
        "governance_controls",
        "Why debate controls whose conformance can be mechanically evaluated?",
        (
            ("policy_violation",),
            ("authorization_denied",),
            ("configuration_drift",),
    ...
    CaseStudy(
        "business_operations",
        "Why route recurring workflow-state failures through meetings and tickets?",
        (
            ("process_stuck",),
            ("dependency_unreachable",),
            ("queue_lag",),
...
def execute_case(
    reasoner: CompiledIssueReasoner, case: CaseStudy, repeats: int
) -> dict[str, object]:
...
def validate(rows: Iterable[dict[str, object]], repeats: int) -> None:
    ...
    for row in rows:
        ...
        assert (
            abs(float(row["portfolio_compiled_coverage"]) - (compiled / calls)) < 1e-12
        )
        ...
        assert (
            int(row["matched"])
            + int(row["refused_evidence"])
            + int(row["fallback_novelty"])
            == expected
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("challenger-issue-reasoning.csv")
    )
    ...
    summary = {
        ...
        "median_calls_per_second": median(
            float(row["calls_per_second"]) for row in rows
        ),
        "total_hypotheses_eliminated": sum(
            int(row["hypotheses_eliminated"]) for row in rows
        ),
