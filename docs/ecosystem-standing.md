...
|---|---|---|---|
| **SA2A bridge** (`src/autofde_lab/beam/beam_port_bridge.py`, `tests/beam/test_sa2a_port_ops_chicago.py`) | `ALIVE` | `cd ~/xaas-worktrees/sjira/sj-006 && /Users/sac/autofde-lab/.venv/bin/python -m pytest tests/beam/ -v` | Run this session: **`4 passed in 1.43s`**, exit 0 (`tests/beam/test_beam_port_bridge.py::test_beam_port_bridge_ping_and_allocate`, plus 3 tests in `test_sa2a_port_ops_chicago.py` covering admit/plan/execute/replay and unknown-op refusal). Note: this worktree has no local `.venv`; the run used `~/autofde-lab/.venv/bin/python` against this worktree's own `tests/beam/` and `src/`, which pytest's `rootdir` resolution confirmed (`rootdir: /Users/sac/xaas-worktrees/sjira/sj-006`). Plain `python3 -m pytest` (system Python 3.14, no venv) fails all 4 with `ModuleNotFoundError: No module named 'autofde_lab'` in the subprocess under test — recorded as a real negative for that invocation, not evidence against the bridge itself. |
...
(`~/xaas-worktrees/sjira/sj-006`), and SJ-006's `path_scope` is exactly
