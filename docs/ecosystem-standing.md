...
| G5 | commitment → actuation (`commit_and_execute`) | autofde-lab + gymact | `PARTIAL_ALIVE` | Real actuation against the real `CubeCounterProvider`: `independently_verified=True`, `final_state={'counter': 3, 'target': 3, 'reward': 1.0, 'solved': True}`. `commit_and_execute` is the **only** actuation path, and it refuses a raw plan tuple with `ADVISORY_AUTHORITY_USED_AS_BEARER`. **Corrected 2026-09-21:** this row previously read `PARTIAL_ALIVE` "because of the defect in the next row" — that defect (G5b) is now fixed (see below), so it no longer justifies `PARTIAL_ALIVE` here. This row stays `PARTIAL_ALIVE` for a *different*, newly-found reason: `run_real_trial`'s fuller `Level4AliveEvidence` construction hits the version-drift Receipt-schema defect named in G5b's evidence column. The evidence this row itself cites (`commit_and_execute` called directly, not through `run_real_trial`) does not hit that path and is unaffected by either defect. |
| G5b | per-step postcondition checking inside `execute_verified` | autofde-lab | `ALIVE` | **Corrected 2026-09-21** — the `SAME_POSTCONDITION_CHECKED_EVERY_STEP` defect this row named is fixed in the real production path, not merely in a test: `run_real_trial` predicts one postcondition per plan step (`predict_step_postconditions`, `level4_crown.py:1747-1753`) and `commit_and_execute` checks step `i`'s real actuation against `expected_list[i]` — not a broadcast terminal value — via `step_expected = expected_list[i]` then `execute_verified(gym, intent, step_expected)` (`level4_crown.py:426-428`). Reached in normal operation from `crown_reconstruct.reconstruct_identity` (`crown_reconstruct.py:137`), not only from a test. Verified this session by re-running `tests/domains/python/test_level4_crown_real_trial_chicago.py` for real, through the real `~/gymact` subprocess bridge and the real `cube_counter` provider (after installing gymact's own `cube` extra, which the sandbox lacked): `test_every_step_of_a_multi_step_plan_is_alive_and_verified` and `test_single_dict_expected_stays_backward_compatible` both **pass**, with `[t["standing"] for t in transitions] == ["ALIVE","ALIVE","ALIVE"]`, `[t["verified"] for t in transitions] == [True,True,True]`, `independently_verified=True`, `ocel_valid=True` — all three steps of a 3-step plan independently checked, not one broadcast expectation. Command: `.venv/bin/python -m pytest tests/domains/python/test_level4_crown_real_trial_chicago.py -v` → `4 passed, 2 failed` (up from `2 passed, 4 failed` before the `cube` extra was installed). **New, separate, NOT-G5b defect surfaced by the same run — not swept into this row:** the remaining 2 failures (`test_run_real_trial_end_to_end`, `test_two_trials_do_not_share_evidence`) are `pydantic_core.ValidationError: 1 validation error for Receipt / planning_provenance_digest / Extra inputs are not permitted [extra_forbidden]` at `level4_crown.py:110`, inside `_standing_from_bridge_result` — a version drift between autofde-lab's pinned `gymact` git dependency (resolves to `26.8.8` installed) and the live `~/gymact` checkout actually used for actuation (`26.8.23`, commit `4ab72e6`, 2026-09-12), whose `Receipt` model (`extra="forbid"`) gained a field the older pinned schema doesn't know. Deliberately not fixed here: it requires a version-bump decision across two repos and its own verification pass, not a bounded change, and it does not touch per-step postcondition checking. Needs its own gap. |
...
- **Retracted 2026-09-21** — "Multi-step `execute_verified` succeeding today → would contradict
  G5b; it does not." No longer true: re-run this pass shows it does succeed (see G5b). Kept
  visible per this file's own rule that corrections are written beside the claim, not edited
  away.
- A real per-step mismatch (any `transitions[i].standing != ALIVE` for a step whose actuation
  matched `expected_list[i]`, or any intermediate step incorrectly `REFUSED` on a plan that does
  match its per-step prediction) → would contradict G5b's `ALIVE` claim; not observed this pass.
...
## Pass 5 — SA2A / XaaS / zcode / ggen_igniter / Semantic Jira fold-in (2026-09-22, SJ-006)

**This section records a linkage that did not previously exist in this ledger.** Before this
pass, this file named none of SA2A, XaaS, zcode, `ggen_igniter`, or Semantic Jira. Every row
below is drawn from a command run this session (autofde-lab side) or a real, inspected commit
on an unmerged branch in the sibling `~/xaas` checkout (xaas side) — never from a claim taken
on faith. Where a claimed standing on disk disagreed with the freshest evidence found (an
unmerged worktree commit postdating the on-disk ticket file), the freshest real evidence wins
and the discrepancy is named below, not silently resolved.

| System | Standing | Command | Evidence |
|---|---|---|---|
| **SA2A bridge** (`src/autofde_lab/beam/beam_port_bridge.py`, `tests/beam/test_sa2a_port_ops_chicago.py`) | `ALIVE` | `cd ~/autofde-lab && git merge-base --is-ancestor 455cc7b5 master && .venv/bin/python -m pytest tests/beam -q` | Run at the merged SHA in this repository: **`4 passed`**, exit 0 — `tests/beam/test_beam_port_bridge.py::test_beam_port_bridge_ping_and_allocate`, plus 3 tests in `test_sa2a_port_ops_chicago.py` covering admit/plan/execute/replay and unknown-op refusal. Subject: autofde-lab master at merge commit `1c25fdc7` (v26.9.22 lane L4 landing, 2026-09-22), which folds in SJ-006 (`sjira/sj-006` 455cc7b5 — this row's own predecessor cited a run from a mixed xaas-side worktree checkout, not this repository; per the SJ-006 fold-in work order AFDE-26922-01, the citation now names a run inside `~/autofde-lab` itself at the merged SHA, with the repository checkout's own `.venv` against its own `tests/beam/` and `src/` — pytest `rootdir: /Users/sac/autofde-lab`). The venv resolves `autofde_lab` to this checkout's `src/autofde_lab` (editable install verified before the run). |
| **XaaS** — SJ-001 `xaas-semantic-jira-e2e` (admit → materialize → receipt → replay) | `ALIVE` | `cd ~/xaas && mix test test/xaas/ultracode && mix xaas.semantic.materialize --help` | Evidence: `~/xaas/worktrees/sjira/sj-001/docs/sjira/v26.9.21/receipts/SJ-001.md`, observed on xaas branch `sjira/sj-001` (head `41e28e5`, base `01aa6fb`) — `mix test test/xaas/ultracode`: **535 tests, 0 failures (21 excluded)**, exit 0; `mix xaas.semantic.materialize --help`: exit 0. `mix compile --warnings-as-errors` exit 0, `mix format --check-formatted` exit 0, mock grep zero matches. Admit → materialize → receipt → replay chain observed end to end: admitted digest `sha256:4e045893…`, receipt outcome `alive`/`head_verified: true`, replay digest reproduced from disk and from a DB re-export, SA2A replay of the receipt manifest `verified: true`. Scope note carried from that receipt: the `mix semantic_jira.*` tasks / `Descriptor.receipt_from_xaas/2` were not present in the `~/ggen_igniter` checkout used, so that specific graph-side reconciliation edge is `UNSUPPORTED` and was not exercised — the rest of the chain (admit/materialize/receipt/replay) is the `ALIVE` claim. **Discrepancy noted**: `~/xaas/docs/sjira/v26.9.21/001-xaas-semantic-jira-e2e.md` and `index.json` on the `xaas` repo's current branch still read `PARTIAL_ALIVE` — this `ALIVE` result lives on the unmerged `sjira/sj-001` worktree branch and has not yet been folded back into `~/xaas`'s own ticket/index; that fold-in is outside this ticket's `path_scope` (`docs/ecosystem-standing.md` in `autofde-lab` only) and is reported back rather than performed here. |
| **XaaS / zcode** — SJ-002 `zcode-ocel-pack-consumer` | `PARTIAL_ALIVE` | `cd ~/xaas && mix ggen_igniter.sync --pack-dir priv/packs/xaas_zcode_ocel_pack ... && mix test test/xaas/telemetry` | Evidence: `~/xaas/worktrees/sjira/sj-002` commit `89143c3` ("feat(sjira/sj-002): generate zcode-ocel-pack registry, wire validator (PARTIAL_ALIVE)", branch `sjira/sj-002`, unmerged). Real `mix ggen_igniter.sync` (oxigraph engine, 4 SPARQL queries, 63 rows) rendered `lib/xaas/generated/zcode_event_registry.ex`, receipted `ALIVE` in `.ggen_igniter/receipts/2026-09-22.jsonl`. Hand-written glue `lib/xaas/telemetry/zcode_ocel_validator.ex` validates an event/object type against the generated registry and refuses an unknown one; `test/xaas/telemetry/zcode_ocel_validator_test.exs` (6 tests) proves the ticket's exact falsifier — unknown event type refused, not accepted. Full `test/xaas/telemetry`: **14 tests, 0 failures**, no mocks (grep-verified). Explicitly **not** `ALIVE`: `priv/zcode_plugin`'s two Node scripts emit zero OCEL events today (zero `ocel` references in either file) — there is no existing zcode-loop OCEL emission call site in the repo to wire the new validator into yet. The registry and validator are real, tested, and ready for that call site once it exists; the wiring itself is the open gap. **Discrepancy noted**: `~/xaas/docs/sjira/v26.9.21/002-zcode-ocel-pack-consumer.md` and `index.json` on the `xaas` repo's current branch still read `UNSUPPORTED` — this `PARTIAL_ALIVE` result lives on the unmerged `sjira/sj-002` worktree branch and has not yet been folded back; reported here as a constraint, not performed across repos. |
| **`ggen_igniter`** — `GgenIgniter.SemanticJira` (WorkOrder admission/selection) | `PARTIAL_ALIVE` | `WO=<wo.json> mix run docs/sjira/v26.9.21/admit.exs` (from `~/ggen_igniter`) | `GgenIgniter.SemanticJira.admit_work_order/1` and `definition_digest/1` are real and were exercised by the SJ-001 receipt above (admitted digest `sha256:4e045893…`). Not `ALIVE` end to end: per the SJ-001 receipt's own scope note, the `mix semantic_jira.*` tasks and `Descriptor.receipt_from_xaas/2` were absent from the `~/ggen_igniter` checkout used this pass (branch `feat/zcode-ocel-pack`), so the graph-side receipt-reconciliation edge is `UNSUPPORTED`, not exercised. |
| **Semantic Jira** (`docs/sjira/v26.9.21/`, this work-order protocol itself) | `PARTIAL_ALIVE` | `python3 -c "import json; d=json.load(open('index.json')); print([(x['id'],x['standing']) for x in d])"` (from `~/xaas/docs/sjira/v26.9.21/`) | Run this session against the committed `~/xaas/docs/sjira/v26.9.21/index.json` (9 orders): `SJ-001 PARTIAL_ALIVE, SJ-002 UNSUPPORTED, SJ-003 PARTIAL_ALIVE, SJ-004 PARTIAL_ALIVE, SJ-005 PARTIAL_ALIVE, SJ-006 PARTIAL_ALIVE, SJ-007 BLOCKED, SJ-008 PARTIAL_ALIVE, SJ-009 UNKNOWN`. Zero orders `ALIVE` on the committed index as of this pass; the SJ-001/SJ-002 real upgrades documented above exist only on unmerged worktree branches (`sjira/sj-001` at `41e28e5`, `sjira/sj-002` at `89143c3`) and have not been folded back into `~/xaas`'s committed `index.json`. The protocol itself (admit → work in worktree → run the runnable check → report standing → commit `-F`) is real and was followed by both of those worktree commits, but the ledger's own machine index is stale relative to them — the system that is supposed to keep it current has not yet closed that loop for SJ-001/SJ-002. |

**Fold-in scope note.** This session cannot edit `~/xaas/docs/sjira/v26.9.21/{001,002}-*.md` or
`index.json` — that is a different repository from this worktree
(the SJ-006 execution worktree), and SJ-006's `path_scope` is exactly
`["docs/ecosystem-standing.md"]`. The `sjira/sj-001` and `sjira/sj-002` worktree branches inside
`~/xaas/worktrees/sjira/` already carry the real upgraded standings and updated ticket files
...
Driven from `autofde-lab` (`src/autofde_lab/iec/`, spec `docs/jira/v26.9.23/`). Every
...
| `autofde-lab@ae255ad1` — held-out C3 subject | `ALIVE` (C3, compared fields) | `python -m autofde_lab.iec.c3 --autofde-lab . --c3-dir receipts/v26.9.23/iec/c3 --ggen-igniter <ggen_igniter>` | Frozen mechanized audit vs an independent LLM hand audit of 13 `ggen.toml` targets: 13/13 agree on (`committed`, `class`); replay reproduces the frozen outcome. See `docs/STATUS.md` pass 46 for the ruff-F401 finding. |
