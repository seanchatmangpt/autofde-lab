...
**Measured win** — test suite, real siblings present:
`IEC_GGEN_IGNITER_CHECKOUT=… IEC_GGEN_CREATE_CHECKOUT=… PYTHONPATH=src python -m pytest -v
tests/iec` → **30 passed in 10.64s** (Python 3.12.3, the pr-ci qualification dependency set).
...
regenerated into an isolated directory, and validated: byte identity, git-blob identity, and
static reflexion against the declared EEx template (parsed, never evaluated). Deleting the
four files and inserting the regenerated bytes restores the frozen root tree
`2e95edb9…` (recomputed in Python; nothing written into the sibling). The ADR row template
`| [{{X0}}]({{X1}}) | {{X2}} | Accepted |` was recovered by anti-unification and predicts each
of the 8 rows held-out (8/8). The generated-header family predicts each of its 3 members
held-out from the pack declaration alone (3/3). Replay: two runs, `diff -r` empty.

**Recorded negative** — `BLOCKED_EXECUTION_AUTHORITY`: ggen_igniter's native verifiers
(`mix format --check-formatted`, `mix compile`, `mix test`) were not run. This session's
execution policy refused `mix deps.get && mix compile` on the sibling. Each subject carries a
`ggen_igniter-native/1` court whose three verifiers are `BLOCKED` with that reason.

**Recorded negative** — `expense_approval_reactor.ex`: the raw-render hypothesis is
**falsified** (mandatory literal `return :` absent: the producer runs `Code.format_string!`,
`reconcile_reactor.ex`). The counterexample is kept. The claim for this file stands only
under the separately named, weaker court `iec-c1-static-modulo-formatter-flat/1`
(whitespace and call parentheses deleted on both sides).

**Measured win** — IEC-C3: the mechanized generated-output audit was frozen (file sha256
`4372b88c…`) before a fresh agent hand-audited the 13 `ggen.toml` rule targets under
`src/autofde_lab/**/*.py` at autofde-lab@ae255ad. The mechanized outcome agrees on 13/13 rows
(`committed`, `class`), and replay reproduces outcome `sha256:9bd48576…`. The ledger computes
`RC-GENERATED-OUTPUT-AUDIT` → `RETIRED_FROM_LLM`, scoped to those two fields and the audit's
block-depth-0 ceiling. Cause attribution stays with the LLM
(`RC-PRODUCER-POSTPROCESSING`: `MECHANIZATION_CANDIDATE`). The ggen_igniter calibration court
agrees 9/9, but it is `UNSUPPORTED` by design: the mechanism was fitted to it.

**Finding (this repo)** — 6 of the 13 generated modules at ae255ad are *not* instances of
their declared templates. They are missing template-emitted imports (`from enum import
Enum` ×5, `from typing import Callable` ×1), removed by the pre-commit
`ruff-check --fix --select=F401` hook in sweep `814615f`, whose message says "No semantic
change". The remaining 7 differ only by ruff-format whitespace. `constitution-world` and
`planner-league-catalog` are `mode = "Overwrite"`, so a fresh `ggen sync run` would revert
them to the unformatted raw render. The ruff stage is undeclared in `ggen.toml`.

**Standing by boundary**: IEC census/decompile/court/anti-unification — `ALIVE` (tests
above). IEC-C1 static courts on 4 ggen_igniter subjects — `ALIVE`. IEC-C1 native courts —
`BLOCKED_EXECUTION_AUTHORITY`. IEC-C3 for `RC-GENERATED-OUTPUT-AUDIT` — `ALIVE` on the
compared fields. IEC-C2 (≥3-repository collapse) and IEC-C4 (ecosystem census) — not
started, `UNKNOWN`. Tree-sitter structural extraction (PR-004) — not implemented. No C1 claim
here is a structural or behavioural equivalence claim. `organizationalStanding` — `UNKNOWN`.

**Falsifiers**: a committed receipt that no longer replays byte for byte; a held-out subject
where the frozen audit and an independent LLM audit disagree on (`committed`, `class`); a
ggen_igniter render (once a broker runs Mix) that differs from the regenerated bytes.
