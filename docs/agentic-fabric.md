...
- **Raise** evidence by binding domain, solver, arguments, subject, policy, and environment identities.
...
Material cache identity includes:
...
Changing any material identity produces a miss. Deterministic refusals are cached for five minutes; transient execution failures are not.
