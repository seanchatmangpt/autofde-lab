...
| 1 | 0.027 | 13 | 6 | 7 | (none) |
| 2 | 0.025 | 12 | 4 | 8 | (none) |
| 3 | 0.034 | 15 | 5 | 10 | (none) |
| 4 | 0.025 | 13 | 5 | 8 | (none) |

**Real numbers across all 5 trials**: 65 total `boundary.execute()` calls that reached actuation (authority+admission passed, fresh idempotency token), 26 records actually present in the shared journal file afterward, **39 lost journal records** (5/5 trials showed at least one lost record).
...
**Per-episode outcome counts across 40 total thread-runs (5 trials x 8 threads)**: 25 reached Episode 1 KNOWN/EXECUTED; 15 attempted actuation but the boundary reported failure (the lost-journal-record consequence above, when nonzero); 0 raised an uncaught exception; 0 silent wrong-answer(s) (cross-episode receipt digest collision) observed.
