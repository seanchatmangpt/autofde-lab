...
def add_element(
    ...
    uri = BNode(
        slug(base.split(":diagram:")[-1].split(":")[0]) + "-e-" + slug(local_id)
    )
...
def ensure_node(
    ...
    local_id,
    label=None,
    cls=ARCH.ArchitectureElement,
    line_no=0,
):
    if local_id not in nodes:
        nodes[local_id] = add_element(
            g, diagram, base, local_id, label or local_id, cls, line_no
        )
...
def parse_c4(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        m = re.match(
            r"(?P<fn>Person|System|System_Ext|Container|Component|Deployment_Node|System_Boundary|Container_Boundary)\((?P<args>.*)\)\s*\{?$",
            line,
        )
        if m:
...
def parse_sequence(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        if m:
            ...
            nodes[m.group(2)] = add_element(
                g, diagram, base, m.group(2), m.group(3).strip(), cls, n
            )
            continue
        ...
        if m:
            ...
            src = ensure_node(
                g, diagram, base, nodes, src_id, cls=ARCH.Participant, line_no=n
            )
            dst = ensure_node(
                g, diagram, base, nodes, dst_id, cls=ARCH.Participant, line_no=n
            )
            seq += 1
            rel_idx += 1
            add_relationship(
...
def parse_state(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        if not m or line.startswith("stateDiagram"):
            continue
        src_id, dst_id, label = [
            x.strip() if x is not None else None for x in m.groups()
        ]
...
def parse_flow(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        def parse_node(expr):
            ...
            node_id = mm.group(1)
            labelv = (
                next((x for x in mm.groups()[1:] if x is not None), None) or node_id
            )
            labelv = labelv.replace("<br/>", " / ")
            return node_id, labelv

        s_id, s_label = parse_node(left)
        d_id, d_label = parse_node(right)
...
def parse_class(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        if m:
            current = m.group(1)
            nodes[current] = add_element(
                g, diagram, base, current, current, ARCH.ModelClass, n
            )
            continue
        if line == "}":
            current = None
        ...
        if current and (
            ...
            attr = BNode(
                slug(base.split(":diagram:")[-1].split(":")[0])
                + "-cm-"
                + slug(current)
                + "-"
                + hashlib.sha1(line.encode()).hexdigest()[:10]
            )
            ...
            continue
        m = re.match(
            r'([A-Za-z0-9_]+)(?:\s+"([^"]*)")?\s+([^\s]+)\s+(?:"([^"]*)"\s+)?([A-Za-z0-9_]+)(?:\s*:\s*(.*))?$',
            line,
        )
        if m and any(x in m.group(3) for x in ["--", "..", "<|", "*--", "o--"]):
            s_id, s_card, notation, d_card, d_id, label = m.groups()
            src = ensure_node(
                g, diagram, base, nodes, s_id, cls=ARCH.ModelClass, line_no=n
            )
            dst = ensure_node(
                g, diagram, base, nodes, d_id, cls=ARCH.ModelClass, line_no=n
            )
            rel_idx += 1
            rel = add_relationship(
                ...
                src,
                dst,
                ARCH.ClassRelationship,
                label,
                n,
                notation=notation,
            )
            if s_card:
                g.add((rel, ARCH.sourceCardinality, Literal(s_card)))
            if d_card:
                g.add((rel, ARCH.targetCardinality, Literal(d_card)))
    return nodes, rel_idx
...
def parse_er(g, diagram, base, text):
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        if m:
            current = m.group(1)
            nodes[current] = add_element(
                g, diagram, base, current, current, ARCH.DataEntity, n
            )
            continue
        if line == "}":
            current = None
        ...
        if current and line:
            ...
            continue
        m = re.match(
            r"([A-Z][A-Z0-9_]*)\s+([^\s]+)\s+([A-Z][A-Z0-9_]*)\s*:\s*(.+)$", line
        )
        if m:
            s_id, notation, d_id, label = m.groups()
            src = ensure_node(
                g, diagram, base, nodes, s_id, cls=ARCH.DataEntity, line_no=n
            )
            dst = ensure_node(
                g, diagram, base, nodes, d_id, cls=ARCH.DataEntity, line_no=n
            )
            rel_idx += 1
            add_relationship(
                g,
...
def build(source: Path) -> Graph:
    ...
    g.add(
        (diagram, ARCH.sourceDigest, Literal(hashlib.sha256(text.encode()).hexdigest()))
    )
    ...
    g.add(
        (
            ...
            PROV.wasDerivedFrom,
            URIRef(
                f"https://github.com/seanchatmangpt/autofde-lab/blob/master/{rel_source}"
            ),
        )
    )
...
def group_for(source: Path) -> str:
    ...
    if 1 <= n <= 5:
        return "01_c4_ecosystem"
    if 6 <= n <= 10:
        return "02_c4_sota_lab"
    if 11 <= n <= 15:
        return "03_c4_gymact"
    if 16 <= n <= 20:
        return "04_c4_autofde_runtime"
    if 21 <= n <= 25:
        return "05_c4_ggen"
    if 26 <= n <= 30:
        return "06_sequence"
...
def build_detail_bundle(name: str, sources: list[Path]) -> Graph:
    ...
    g.add(
        (
            ontology,
            DCTERMS.title,
            Literal(f"Generated detailed architecture graph bundle: {name}"),
        )
    )
    for src in sources:
        sg = build(src)
...
def build_catalog_bundle(name: str, sources: list[Path]) -> Graph:
    ...
    g.add(
        (
            ontology,
            DCTERMS.title,
            Literal(f"Generated architecture catalog bundle: {name}"),
        )
    )
    for src in sources:
        sg = build(src)
...
def grouped_sources() -> dict[str, list[Path]]:
    ...
    if len(groups) != 10 or any(len(v) != 5 for v in groups.values()):
        raise SystemExit(
            f"expected ten groups of five, got {[(k, len(v)) for k, v in groups.items()]}"
        )
    return groups
