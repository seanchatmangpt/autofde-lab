...
async def main() -> int:
    ...
    gate = CapabilityGate.from_toml(
        REPO_ROOT / "src" / "autofde_lab" / "fabric" / "gymact_capabilities.toml"
    )

    print(
        f"[1/7] Materializing real SregymEnvironment for problem_id={problem_id!r} ..."
    )
    ...
    try:
        ...
        deployments = await _kubectl_json(
            env, gate, f"get deployments -n {namespace} -o json"
        )
        pods = await _kubectl_json(env, gate, f"get pods -n {namespace} -o json")
        services = await _kubectl_json(
            env, gate, f"get services -n {namespace} -o json"
        )
        ...
        print(
            f"[4/7] {len(anomalies)} real anomalies found: {[a.kind for a in anomalies]}"
        )

        if not anomalies:
            print(
                "[4/7] No anomaly found -- honest 'no diagnosis', not forced. Stopping here."
            )
        ...
        print(
            f"[5/7] diagnosis submitted: {json.dumps(diag_result.get('after', {}), indent=2)[:500]}"
        )

        print(
            "[6/7] No automated remediation command derived yet for this anomaly class "
        ...
        print(
            f"[6/7] mitigation result: {json.dumps(mit_result.get('after', {}), indent=2)[:500]}"
        )

        print("[7/7] verify() -- real bounded poll against the real conductor ...")
        passed, observed = await env.verify({"stage": "complete"})
        print(
            f"[7/7] verify passed={passed} observed={json.dumps(observed, indent=2)[:500]}"
        )
